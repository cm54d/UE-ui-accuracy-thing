...
-- Background tab content, shown next to the Themes tab below.
-- All Callback stubs are empty — wire these up to your actual background
-- item / post-processing effects. Adjust the Item/Material dropdown lists
-- and the Min/Max/Default values to match your setup.
local function CreateBackgroundTab(BackgroundTab)
	BackgroundTab:AddLabel("Item Color"):AddColorPicker("Background_ItemColor", {
		Default = Color3.fromRGB(255, 255, 255),
		Callback = function(Value)
			-- e.g. BackgroundItem.Color = Value
		end,
	})

	BackgroundTab:AddDropdown("Background_Item", {
		Text = "Item",
		Values = { "Mourne" }, -- add your other background item names here
		Default = "Mourne",
		Callback = function(Value)
			-- e.g. swap the active background item/mesh here
		end,
	})

	do
		local TransparencyCol, ReflectanceCol = BackgroundTab:AddRow(2)
		TransparencyCol:AddSlider("Background_ItemTransparency", {
			Text = "Transparency", Default = 0, Min = 0, Max = 1, Rounding = 2,
			Callback = function(Value)
				-- e.g. BackgroundItem.Transparency = Value
			end,
		})
		ReflectanceCol:AddSlider("Background_ItemReflectance", {
			Text = "Reflectance", Default = 0, Min = 0, Max = 1, Rounding = 2,
			Callback = function(Value)
				-- e.g. BackgroundItem.Reflectance = Value
			end,
		})
	end

	BackgroundTab:AddSlider("Background_ItemSpeed", {
		Text = "Speed", Default = 0.15, Min = 0, Max = 1, Rounding = 2,
		Callback = function(Value)
			-- e.g. rotation/animation speed of the background item
		end,
	})

	BackgroundTab:AddDropdown("Background_ItemMaterial", {
		Text = "Material",
		Values = { "Plastic", "SmoothPlastic", "Neon", "Glass", "Metal", "Marble", "Wood", "Ice" },
		Default = "Neon",
		Callback = function(Value)
			-- e.g. BackgroundItem.Material = Enum.Material[Value]
		end,
	})

	BackgroundTab:AddDivider()

	BackgroundTab:AddLabel("Background Color"):AddColorPicker("Background_TintColor", {
		Default = Color3.fromRGB(0, 0, 0),
		Callback = function(Value)
			-- e.g. ColorCorrection.TintColor = Value
		end,
	})

	do
		local TransparencyCol, BlurCol = BackgroundTab:AddRow(2)
		TransparencyCol:AddSlider("Background_TintTransparency", {
			Text = "Transparency", Default = 0.7, Min = 0, Max = 1, Rounding = 2,
			Callback = function(Value)
				-- e.g. dim overlay's Transparency
			end,
		})
		BlurCol:AddSlider("Background_Blur", {
			Text = "Blur", Default = 15, Min = 0, Max = 50, Rounding = 0,
			Callback = function(Value)
				-- e.g. BlurEffect.Size = Value
			end,
		})
	end

	do
		local ContrastCol, SaturationCol = BackgroundTab:AddRow(2)
		ContrastCol:AddSlider("Background_Contrast", {
			Text = "Contrast", Default = 0, Min = 0, Max = 10, Rounding = 1,
			Callback = function(Value)
				-- e.g. ColorCorrection.Contrast = Value / 10
			end,
		})
		SaturationCol:AddSlider("Background_Saturation", {
			Text = "Saturation", Default = 0, Min = 0, Max = 10, Rounding = 1,
			Callback = function(Value)
				-- e.g. ColorCorrection.Saturation = Value / 10
			end,
		})
	end

	BackgroundTab:AddSlider("Background_Brightness", {
		Text = "Brightness", Default = 0, Min = 0, Max = 1, Rounding = 2,
		Callback = function(Value)
			-- e.g. ColorCorrection.Brightness = Value
		end,
	})
...
-- inside a Tabbox alongside a "Background" tab (instead of ThemeManager:ApplyToTab,
-- which would give Themes its own standalone groupbox with no room for a second tab)
...
local BackgroundTab = SettingsTabbox:AddTab("Background")
CreateBackgroundTab(BackgroundTab)
